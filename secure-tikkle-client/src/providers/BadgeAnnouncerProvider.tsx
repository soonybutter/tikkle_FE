...
import { BadgeAnnouncerContext } from './badgeAnnouncerContext';
...
export default function BadgeAnnouncerProvider({ children }: { children: React.ReactNode }) {
  ...
  // 최초 1회: 과거 배지 본 것으로 마킹
  useEffect(() => {
    (async () => {
      if (localStorage.getItem(INIT_KEY)) return;
      const list = await Badges.list();
      const initial: Record<string, string> = {};
      list.forEach((b) => { if (b.earned && b.earnedAt) initial[b.code] = b.earnedAt; });
  ...
  const scan = useCallback(async () => {
    ...
    const newly = list.filter((b) => b.earned && b.earnedAt && seen[b.code] !== b.earnedAt);
    if (newly.length) {
      const nextSeen = { ...seen };
      newly.forEach((b) => { if (b.earnedAt) nextSeen[b.code] = b.earnedAt; });
      localStorage.setItem(SEEN_KEY, JSON.stringify(nextSeen));
      newly.forEach((b) => push(b));
    }
  }, [push]);

  const value = useMemo(() => ({ triggerScan: scan }), [scan]);
...
function Modal({ children, onClose }: { children: React.ReactNode; onClose: () => void }) {
  ...
  return (
    <motion.div className={modalCss.backdrop}
      ...
      onClick={onClose}>
      <motion.div className={modalCss.card}
...
function BadgeCard({ badge, onClose }: { badge: BadgeDto; onClose: () => void }) {
  return (
    <div className={modalCss.inner}>
      ...
      {badge.earnedAt && <div className={modalCss.meta}>획득일: {new Date(badge.earnedAt).toLocaleString()}</div>}
